...
using namespace gem5::enums;

#include "arch/riscv/regs/misc.hh"
#include "debug/ValueCommit.hh"

uint8_t *pmemStart = nullptr;
uint64_t pmemSize = 0;
...
skipPerfCntCsr()
{
    ...
    for (uint64_t counter = gem5::RiscvISA::CSR_HPMCOUNTER03;
                  counter <= gem5::RiscvISA::CSR_HPMCOUNTER31; counter++) {
        skipCSRs.push_back(GetCSROPInstCode(counter));
    }
    for (uint64_t counter = gem5::RiscvISA::CSR_MHPMEVENT03;
