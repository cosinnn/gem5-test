...
EStride::EStride(const Params &params)
    : VPUnit(params),
      ...
      confidenceThreshold(static_cast<int>(
          params.thresholdPercent * MAXCONFIDENCE)),
      ...
      esstats(this)
{
    ...
    gem5_assert(params.inflightWindowTagLength,
                "EStride inflightWindowTagLength must > 0 \n");

    inflightWindows.reserve(numThreads);
    for (ThreadID tid = 0; tid < numThreads; ++tid) {
        inflightWindows.emplace_back(params.inflightWindowTagLength,
                                     params.idealWindow);
    }
    ...
    DPRINTF(EStride,
            "================================== EStride Params ==================================\n");
    DPRINTF(EStride,
            "ways: %7d | strideWidth: %7d bits | tagWidth %7d bits\n",
            ways, strideWidth, tagWidth);
    DPRINTF(EStride, "EStride table entrys: %7d\n", entryCounts);
    DPRINTF(EStride,
            "MAXCONFIDENCE: %7d | confidenceThreshold %7d\n",
...
{
    return ((tag1 & ((1 << tagWidth) - 1)) ^
            (tag2 & ((1 << tagWidth) - 1)));
}

EStride::UpdateConfDecision
EStride::decideToUpdate(const ESUpdateMetaData *esUpdateMetaData,
                        int64_t stride)
{
    ...
    bool finalUpdate =
        shouldUpdate & ((std::abs(stride) > 1) ||
                        !esUpdateMetaData->isLoadInst ||
                        ((stride == -1) &
                         ((rng->random<int32_t>() & 1) == 0)) ||
                        ((stride == 1) &
                         ((rng->random<int32_t>() & 3) == 0)));
...
{
    uint32_t k = 2 +
        2 * (entry.confidence > MAXCONFIDENCE / 8) +
        2 * (entry.confidence >= MAXCONFIDENCE / 4);
    uint32_t mask = (1 << k) - 1;

    return rng->random<uint32_t>() & mask;
}
...
EStride::doPredict(ESPredMetaData *esPredMetaData, int inflights)
{
    ...
    if (debug::EStride) {
        ...
        DPRINTF(EStride,
                "[ESPredict] seq_no:%lu pc: %lX index each way: %s\n",
                esPredMetaData->seq_no, esPredMetaData->pc,
                indexMarker.c_str());
        DPRINTF(EStride,
                "[ESPredict] seq_no:%lu pc: %lX tag each way: %s\n",
                esPredMetaData->seq_no, esPredMetaData->pc,
                tagMarker.c_str());
    }
    ...
    if (!found) {
        DPRINTF(EStride,
                "[ESPredict]=> seq_no:%lu pc:%lX not found in ES\n",
                esPredMetaData->seq_no, esPredMetaData->pc);
        return {false, 0};
    }

    DPRINTF(EStride,
            "[ESPredict][way: %d index: %u][confidence: %d  useful: %d lastValue: %lu]\n",
            way, index, entryCopy.confidence, entryCopy.useful,
            entryCopy.lastValue);

    uint64_t predValue = (uint64_t)((int64_t)entryCopy.lastValue +
                         (inflights + 1) * extendStride(entryCopy.stride));
    ...
    if (entryCopy.confidence < confidenceThreshold) {
        DPRINTF(EStride,
                ...
                esPredMetaData->seq_no, esPredMetaData->pc, way, index,
                inflights);
        return {false, predValue};
    }

    DPRINTF(EStride,
            "[ESPredict]=> seq_no:%lu pc:%lX get prediction in [way: %d index: %u inflights: %d]\n",
            esPredMetaData->seq_no, esPredMetaData->pc, way, index,
            inflights);
...
EStride::valuePredict(VPPredMetaData *predMetaData)
{
    ...
    ESPredMetaData *esPredMetaData =
        dynamic_cast<ESPredMetaData *>(predMetaData);
    assertValidTid(esPredMetaData->tid);

    int inflights =
        inflightWindows[esPredMetaData->tid].addToInflightWindow(
            esPredMetaData->pc);
...
EStride::updateValuePredictor(VPUpdateMetaData *updateMetaData)
{
    ...
    ESUpdateMetaData *esUpdateMetaData =
        dynamic_cast<ESUpdateMetaData *>(updateMetaData);
    assertValidTid(esUpdateMetaData->tid);
    const ThreadID tid = esUpdateMetaData->tid;

    inflightWindows[tid].removeFromWindow(esUpdateMetaData->pc,
                                          esUpdateMetaData->seq_no);
    ...
    if (debug::EStride) {
        ...
        DPRINTF(EStride,
                "[ESUpdate] seq_no:%lu pc: %lX index each way: %s\n",
                updateMetaData->seq_no, updateMetaData->pc,
                indexMarker.c_str());
        DPRINTF(EStride,
                "[ESUpdate] seq_no:%lu pc: %lX tag each way: %s\n",
                updateMetaData->seq_no, updateMetaData->pc,
                tagMarker.c_str());
    }
    ...
    if (found) {
        ...
        DPRINTF(EStride,
                "[way: %d index: %u][confidence: %d  useful: %d lastValue: %lu]\n",
                way, index, entry.confidence, entry.useful,
                entry.lastValue);

        DPRINTF(EStride,
                "activate value - last value: %lu-%lu, appear before: %s \n",
                esUpdateMetaData->actualValue, entry.lastValue,
                entry.NotFirstAppear ? "not" : "yes");
        bool misprediction =
            !(esUpdateMetaData->actualValue ==
              (uint64_t)((int64_t)entry.lastValue +
              extendStride(entry.stride)));
        int64_t actualStride =
            esUpdateMetaData->actualValue - entry.lastValue;
        entry.lastValue = esUpdateMetaData->actualValue;

        if (entry.NotFirstAppear) {
            DPRINTF(EStride,
                    "activate stride - entry stride = %d - %d \n",
            ...
            if (!misprediction) {
                ...
                if (!fluentInstructions.count(esUpdateMetaData->pc)) {
                    fluentInstructions[esUpdateMetaData->pc] = {
                        1, std::string(esUpdateMetaData->disas)};
                } else {
                    fluentInstructions[esUpdateMetaData->pc].first += 1;
                }

                UpdateConfDecision decision =
                    decideToUpdate(esUpdateMetaData, actualStride);
                if (decision.first) {
                    entry.confidence = std::min(MAXCONFIDENCE,
                                                entry.confidence +
                                                decision.second);
                }
            ...
            DPRINTF(EStride,
                    "[ESFirstStride]before stride: %d => after stride: %d\n",
        ...
        uint32_t wayBegin = rng->random<uint32_t>() % ways;

        for (size_t i = 0; i < ways; ++i) {
            ESEntry &entry = ESTables[tid][wayBegin][indexEachWays[wayBegin]];
            if (entry.confidence == 0) {
                DPRINTF(EStride,
                        "allocate by confidence: [way: %d index: %u] \n",
        ...
        for (size_t i = 0; i < ways; ++i) {
            ...
            if (entry.useful == 0) {
                DPRINTF(EStride,
                        "allocate by useful: [way: %d index: %u] \n",
                        wayBegin, indexEachWays[wayBegin]);
                esstats.allocate[wayBegin][indexEachWays[wayBegin]]++;
                entry.tag = tagEachWays[wayBegin];
                entry.confidence = 1;
        ...
        if (tryDecUseful(entry) == 0) {
            DPRINTF(EStride,
                    "[dec useful count]=> way: %d index: %d",
                    wayBegin, indexEachWays[wayBegin]);
            entry.useful -= 1;
        }
    }
